        ...
        try
        {
            ...
            if(chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
            {
                ...
                while(reader.ready())
                {
                    ...
                    wordBreaker = new StringTokenizer(rec);
                    while ((wordBreaker.hasMoreTokens()))
                    {
                        String token = wordBreaker.nextToken();
                        // Now test the token to be a shortWord and dump it to the console if it is short

                        if (filter.accept(token))
                        {
                            System.out.println(token);
                            AcceptedWords.add(token);
                        }
